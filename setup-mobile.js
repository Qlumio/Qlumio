...
// ─── 1. Opprett DayView.tsx ───────────────────────────────────────────────────

const dayViewContent = `"use client";
...
const dayViewPath = path.join(__dirname, "components", "DayView.tsx");
fs.writeFileSync(dayViewPath, dayViewContent, "utf8");
console.log("✅ Opprettet: components/DayView.tsx");

// ─── 2. Patch WeekGrid.tsx ────────────────────────────────────────────────────

const weekGridPath = path.join(__dirname, "components", "WeekGrid.tsx");
let wg = fs.readFileSync(weekGridPath, "utf8");

// Legg til useEffect i React-import
wg = wg.replace(
  `import { useState } from "react";`,
  `import { useState, useEffect } from "react";`
);

// Legg til DayView-import etter EventActionsModal
wg = wg.replace(
  `import EventActionsModal from "@/components/EventActionsModal";`,
  `import EventActionsModal from "@/components/EventActionsModal";\nimport DayView from "@/components/DayView";`
);

// Legg til isMobile state + useEffect etter hiddenMemberIds
wg = wg.replace(
...
// Legg til mobilsjekk før return
wg = wg.replace(
...
fs.writeFileSync(weekGridPath, wg, "utf8");
console.log("✅ Oppdatert:  components/WeekGrid.tsx");

// ─── 3. Git ───────────────────────────────────────────────────────────────────

const { execSync } = require("child_process");
try {
  execSync("git add components/DayView.tsx components/WeekGrid.tsx", { stdio: "inherit" });
  execSync(`git commit -m "feat: mobilvisning (dagsvisning) for kalender"`, { stdio: "inherit" });
  execSync("git push", { stdio: "inherit" });
  console.log("\n🚀 Pushet til GitHub – Vercel deployer nå!");
} catch (e) {
  console.log("\n⚠️  Git-steg feilet. Kjør manuelt:");
  console.log("   git add .");
  console.log('   git commit -m "feat: mobilvisning for kalender"');
  console.log("   git push");
}

console.log("\n✅ Ferdig! Test på iPhone om 1–2 minutter på qlumio.com");
