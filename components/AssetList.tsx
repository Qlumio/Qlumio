      ...
      <div className="max-w-lg mx-auto">
        ...
        {assets.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-slate-400 mb-4">Ingen eiendeler lagt til ennå.</p>
            <button
              onClick={() => setShowModal(true)}
        ...
        <div className="space-y-3">
          {assets.map((asset) => (
            <Link
              ...
              className="flex items-center gap-4 p-4 bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors group"
            >
              <div className="text-3xl w-12 text-center flex-shrink-0">
                {getAssetEmoji(asset.type)}
              </div>
              <div className="min-w-0">
                <div className="font-semibold group-hover:text-white">{asset.name}</div>
                <div className="text-sm text-slate-400">
                  {getAssetLabel(asset.type)}
                  {asset.purchase_year && <span className="ml-2 text-slate-500">· {asset.purchase_year}</span>}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
          <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 mb-5">
              <div>
                <label className="text-xs text-slate-500 mb-1 block">Navn</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs text-slate-500 mb-1 block">Kjøpsår (valgfritt)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-xs text-slate-500 mb-1 block">Beskrivelse (valgfritt)</label>
                <input
                  type="text"
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowModal(false)} className="flex-1 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 transition-colors text-sm">Avbryt</button>
              <button
                onClick={handleSave}
