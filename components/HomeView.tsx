...
// Moduler tilgjengelig for admin
const ALL_MODULES = [
  {
    href: "/aktiviteter",
    title: "Aktiviteter",
  ...
  },
  {
    href: "/innkjop",
    title: "Innkjøp",
    description: "Dagligvareliste og planlagte innkjøp",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function HomeView({ members }: Props) {
  const { currentUser, setCurrentUser, isLoaded } = useUser();

  // Vent på at localStorage er lastet
  if (!isLoaded) return null;

  // Vis profilvelger hvis ingen er innlogget
  if (!currentUser) {
    return <ProfileSelector members={members} />;
  }

  // Oppdater currentUser med ferske data fra server (i tilfelle noe er endret)
  ...
  // Vis innstillinger hvis admin ELLER hvis ingen admin-bruker finnes ennå (bootstrap)
  const hasAnyAdmin = members.some((m) => m.permission_level === "admin");
  const showSettings = freshUser.permission_level === "admin" || !hasAnyAdmin;
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center justify-between mb-10 pt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 text-sm px-3 py-1.5 rounded-lg hover:bg-white transition-colors group"
              title="Bytt bruker"
            >
              <div
                className={`w-6 h-6 rounded-full ${freshUser.color} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}
              >
                {freshUser.name[0].toUpperCase()}
              </div>
              <span className="text-gray-600 group-hover:text-gray-900 transition-colors hidden sm:block">
                {freshUser.name}
              </span>
            </button>

            {/* Innstillinger – admin eller ingen admin finnes ennå */}
            {showSettings && (
              <Link
                href="/innstillinger"
                className="flex items-center gap-1.5 text-gray-500 hover:text-gray-900 transition-colors text-sm px-3 py-1.5 rounded-lg hover:bg-white"
                title="Innstillinger"
              >
                ...
                <span className="hidden sm:block">Innstillinger</span>
              </Link>
            )}
          </div>
        </div>
        ...
        {/* Om appen – kun admin */}
        {freshUser.permission_level === "admin" && (
          <Link
            href="/om"
